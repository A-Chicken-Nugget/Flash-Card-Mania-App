...
public class CardListing {
    ...
    public CardListing(Context context, LinearLayout layout, Card card) {
        ...
        if (cardFront.getType()) {
            ...
            ((ImageView)imageView).setImageBitmap(BitmapFactory.decodeFile(new File(cardFront.getContents()).getAbsolutePath()));
            panel.findViewById(R.id.cardListing_text).setVisibility(View.INVISIBLE);
            imageView.setVisibility(View.VISIBLE);
        } else {
            ((TextView)panel.findViewById(R.id.cardListing_text)).setText(cardFront.getContents());
        }
    }
